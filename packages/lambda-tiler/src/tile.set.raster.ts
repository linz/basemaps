...
// import { Config, Env, fsa, LogType, titleizeImageryName } from '@basemaps/shared';
...
// import { TiffCache } from './util/source.cache.js';
...
//   public async tile(req: LambdaHttpRequest, xyz: TileXyz): Promise<LambdaHttpResponse> {
//     if (xyz.tileType === VectorFormat.MapboxVectorTiles) return NotFound;
//     const tiffs = await this.initTiffs(xyz.tile, req.log);
//     const layers = await this.tiler.tile(tiffs, xyz.tile.x, xyz.tile.y, xyz.tile.z);

//     // Generate a unique hash given the full URI, the layers used and a renderId
//     const cacheKey = Etag.generate(layers, xyz);
