...
o.spec('FileProcess', () => {
    ...
    o('should extract and track a api hit', async () => {
        ...
        o(logData.byApi.size).equals(1);
        const apiStats = logData.byApi.get(DevApiKey);
    ...
    o('should extract and track a bunch of hits', async () => {
        ...
        o(logData.byApi.size).equals(2);
        const devStats = logData.byApi.get(DevApiKey);
        const clientStats = logData.byApi.get(ClientApiKey);
        ...
        o(clientStats?.cache).deepEquals({ hit: 1, miss: 0 });
        o(clientStats?.projection).deepEquals({ 2193: 0, 3857: 1 });
        o(clientStats?.extension).deepEquals({ webp: 0, jpeg: 0, png: 0, wmts: 1, other: 0 });
        o(clientStats?.tileSet).deepEquals({ aerial: 0, aerialIndividual: 0, topo50: 1, direct: 0 });
    });
});
