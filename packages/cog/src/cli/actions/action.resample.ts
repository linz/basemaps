...
import { buildCogForQuadKey, CogJob } from '../../cog rst';
...
export class ActionResampleCreate extends CommandLineAction {
    ...
    public constructor() {
        super({
            actionName: 'resample',
            summary: 'create a COG using a specific resampling method',
            documentation: 'Create a COG from a vrt for the specified quadkey and resampling method',
        });
    }
    ...
    async onExecute(): Promise<void> {
        ...
        const resampleMethod = this.resampleMethod?.value ?? 'lanczos';
