  ...
  <VRTRasterBand dataType="Byte" band="${i + 1}">
    ...
    ${tifs.map((n) => (c === 'alpha' ? complexSource(n) : simpleSource('/vsis3/' + n, i + 1))).join('\n')}
        ...
        o('1 crosses, 1 outside', async () => {
            const cutline = await Cutline.loadCutline(Epsg.Google, testDir + '/kapiti.geojson');
            const cl2 = await Cutline.loadCutline(Epsg.Google, testDir + '/mana.geojson');
