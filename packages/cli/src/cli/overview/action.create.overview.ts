...
export const DefaultMaxZoom = 15;
...
export class CommandCreateOverview extends CommandLineAction {
  ...
  async onExecute(): Promise<void> {
    ...
    await createOverview(source, maxZoom, logger, output);
  }
}

async function prepareTiles(files: NamedBounds[], maxZoom: number): Promise<Set<string>> {
...
function addChildren(qk: string, maxZoom: number, tiles: Set<string>): void {
  if (qk.length >= maxZoom) return;
  for (const child of QuadKey.children(qk)) {
    tiles.add(child);
    if (child.length < maxZoom) addChildren(child, maxZoom, tiles);
  }
}

async function getTileMatrix(sources: ChunkSource[]): Promise<TileMatrixSet> {
...
async function createTar(path: string, logger: LogType, output?: string): Promise<void> {
...
export async function createOverview(source: string, maxZoom: number, logger: LogType, output?: string): Promise<void> {
  ...
  const tileMatrix = await getTileMatrix(tiffSource);
  ...
  const tiles = await prepareTiles(metadata.files, maxZoom);
  ...
  await createTar(path, logger, output);
