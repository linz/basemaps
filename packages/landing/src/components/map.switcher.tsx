...
export class MapSwitcher extends Component {
  ...
  updateMap = (): void => {
    ...
    if (Config.map.location.bearing) this.map.setBearing(Config.map.location.bearing);
    if (Config.map.location.pitch) this.map.setPitch(Config.map.location.pitch);
    this.forceUpdate();
  };
