...
o.spec('ConfigDynamoVersioned', () => {
    ...
    o.spec('create', () => {
        o('Should create initial tags', async () => {
            ...
            await config.TileSet.create({ name: 'test' } as any);

            o(get.callCount).equals(1);
            o(get.firstCall.firstArg).equals('ts_test_head');
            o(put.callCount).equals(2);
            o(put.firstCall.firstArg.id).equals('ts_test_v000000');
            o(put.secondCall.firstArg.id).equals('ts_test_head');
        });

        o('should increment version number', async () => {
            const put = sandbox.stub(config.TileSet, 'put');
            const get = sandbox.stub(config.TileSet, 'get').callsFake(async () => ({ revisions: 5 } as any));
            const res = await config.TileSet.create({ name: 'test', imagery: {} } as any);

            o(get.callCount).equals(1);
            o(get.firstCall.firstArg).equals('ts_test_head');
            o(put.callCount).equals(2);
            o(put.firstCall.firstArg.id).equals('ts_test_v000006');
            o(put.secondCall.firstArg.id).equals('ts_test_head');
            ...
            const resB = await config.TileSet.create({ name: 'test', imagery: {} } as any);
            o(res.version).equals(6);
    ...
    o.spec('tag', () => {
        o('should create tags', async () => {
            ...
            const res = await config.TileSet.tag({ name: 'test', version: 5 } as any, config.Tag.Production);
            o(res.id).equals('ts_test_production');
            o(get.callCount).equals(0);
            o(put.callCount).equals(1);
            o(put.firstCall.firstArg.id).equals('ts_test_production');
        });
    });
});
